...
int main(int argc, char* argv[]) {
	...
	/* example use of benchmarking */
	{
		...
		benchmark(threadcnt, u8"coarse-grained read", [&l2](int random){
			read(l2, random);
		});
		benchmark(threadcnt, u8"fine-grained update", [&l1](int random){
			update(l1, random);
		});
		benchmark(threadcnt, u8"coarse-grained update", [&l2](int random){
			update(l2, random);
		});
	}
	{
		...
		benchmark(threadcnt, u8"coarse-grained mixed", [&l2](int random){
			mixed(l2, random);
		});
	}
	return EXIT_SUCCESS;
